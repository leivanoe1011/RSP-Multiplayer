    ...
    if(currentRoomKey !== null){
        db2.ref(currentRoomKey).on("value", function(snapshot){

            // Need to check if Messages Change
            loadMessage(snapshot.messages);
    
            // Need to check if player1Waiting changed
            // Need to check if player2Waiting changed
            var player1Waiting = snapshot.player1Waiting;
    
            var player2Waiting = snapshot.player2Waiting;
